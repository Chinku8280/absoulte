...
class Xin_model extends CI_Model
{
	...
	public function check_all_employee_att($status)
	{
		$sql = 'SELECT * FROM xin_employees WHERE attendance_status = ?';
		$binds = array($status);
		$query = $this->db->query($sql, $binds);
		if ($query->num_rows() > 0) {
			return $query->result();
	...
	// get currency sign
	public function get_currency_sign($country_name = '')
	{
		$currencies = [
			"Afghanistan" => "AFN - ؋",
			"Albania" => "ALL - L",
		...
		if (empty($country_name) && $country_name != 0)
			return $currencies;
		else
			return  $currencies[$country_name] ?? 0;
	}



	//set currency sign
	public function currency_sign($number, $user_id = 0)
	{
		...
		$user_info = $this->read_user_info($user_id);
		setlocale(LC_MONETARY, $default_locale);

		$get_currency_sign = $this->get_currency_sign($user_info[0]->currency ?? 0);

		if (isset($user_info) && !is_numeric($user_info[0]->currency) && !is_numeric($get_currency_sign)) {
			if ($system_setting[0]->show_currency == 'code') {
				$ar_sc = explode(' -', $get_currency_sign);
				$sc_show = $ar_sc[0];
			} else {
				$ar_sc = explode('- ', $get_currency_sign);
				$sc_show = $ar_sc[1];
	...
	// get current day attendance 
	public function get_attendance_using_status($attendance_status)
	{

		$session = $this->session->userdata('username');
		$sql = 'SELECT attendance_status FROM xin_employees WHERE attendance_status = ? and is_active = 1';
		$binds = array($attendance_status);
		$query = $this->db->query($sql, $binds);
		return $query->num_rows();
	}
	...
	{
		return $this->db->get('xin_deduction_type');
	}
	public function add_deduction_type($data)
	{
		$this->db->insert('xin_deduction_type', $data);
		if ($this->db->affected_rows() > 0) {
			return true;
		} else {
			return false;
	...
	public function read_deduction_type($id)
	{
		$sql = 'SELECT * FROM xin_deduction_type where deduction_type_id = ?';
		$binds = array($id);
		$query = $this->db->query($sql, $binds);
...
ORDER BY `expiry_date`");
	...
	public function count_get_documents_expired_all()
	{
		...
		$curr_date = date('Y-m-d');
		$query = $this->db->query("SELECT * from xin_employee_documents where STR_TO_DATE(date_of_expiry,'%d-%m-%Y') < '" . $curr_date . "' and date_of_expiry !='' and date_of_expiry is not null ORDER BY `date_of_expiry` asc");
		return $query->num_rows();
	}
	// user/
	...
	public function count_get_img_documents_expired_all()
	{
		...
		$curr_date = date('Y-m-d');
		$query = $this->db->query("SELECT * from xin_employee_immigration where STR_TO_DATE(expiry_date,'%d-%m-%Y') < '" . $curr_date . "'  and expiry_date !='' and expiry_date is not null ORDER BY `expiry_date` asc");
		return $query->num_rows();
	}
	...
	public function count_get_user_img_documents_expired_all($employee_id)
	{
		...
		$curr_date = date('Y-m-d');
		$query = $this->db->query("SELECT * from xin_employee_immigration where employee_id = '" . $employee_id . "' and STR_TO_DATE(expiry_date,'%d-%m-%Y') < '" . $curr_date . "' and expiry_date !='' and expiry_date is not null ORDER BY `expiry_date` asc");
		return $query->num_rows();
	}
	public function iicount_company_license_expired_all()
	{
		$curr_date = date('Y-m-d');
		$query = $this->db->query("SELECT * from xin_company_documents where STR_TO_DATE(expiry_date,'%d-%m-%Y') < '" . $curr_date . "' and expiry_date !='' and expiry_date is not null ORDER BY `expiry_date` asc");
		return $query->num_rows();
	}
	...
	{
		$curr_date = date('Y-m-d');
		$query = $this->db->query("SELECT * from xin_assets where STR_TO_DATE(warranty_end_date,'%d-%m-%Y') < '" . $curr_date . "'  and warranty_end_date !='' and warranty_end_date is not null ORDER BY `warranty_end_date` asc");
		return $query->num_rows();
	}
	...
	{
		$curr_date = date('Y-m-d');
		$query = $this->db->query("SELECT * from xin_assets where employee_id = '" . $employee_id . "' and STR_TO_DATE(warranty_end_date,'%d-%m-%Y') < '" . $curr_date . "' and warranty_end_date !='' and warranty_end_date is not null ORDER BY `warranty_end_date` asc");
		return $query->num_rows();
	}
	// company assets warranty all
	...
	{
		$curr_date = date('Y-m-d');
		$query = $this->db->query("SELECT * from xin_assets where company_id = '" . $company_id . "' and STR_TO_DATE(warranty_end_date,'%d-%m-%Y') < '" . $curr_date . "'  and warranty_end_date !='' and warranty_end_date is not null  ORDER BY `warranty_end_date` asc");
		return $query->num_rows();
	}
	// get client projects
	...
	{
		return  $query = $this->db->query("SELECT * from xin_claim_type");
	}
	// Function to add record in table
	...
	{
		$this->db->insert('xin_claim_type', $data);
		if ($this->db->affected_rows() > 0) {
			return true;
		} else {
			return false;
	...
	// get single record > db table > constant
	public function read_claim_type($id)
	{

		$sql = 'SELECT * FROM xin_claim_type where claim_type_id = ?';
		$binds = array($id);
		$query = $this->db->query($sql, $binds);
	...
	// Function to update record in table
	public function update_claim_type_record($data, $id)
	{
		$this->db->where('claim_type_id', $id);
		if ($this->db->update('xin_claim_type', $data)) {
			return true;
		} else {
			return false;
		}
	}
	public function update_deduction_type_record($data, $id)
	{
		$this->db->where('deduction_type_id', $id);
		if ($this->db->update('xin_deduction_type', $data)) {
			return true;
		} else {
			return false;
		}
	...
	// Function to Delete selected record from table
	public function delete_claim_type_record($id)
	{
		$this->db->where('claim_type_id', $id);
		$this->db->delete('xin_claim_type');
	}
	public function delete_deduction_type_record($id)
	{
		$this->db->where('deduction_type_id', $id);
		$this->db->delete('xin_deduction_type');
	}
	// v1.1.7
	// get all table rows 
	...
	public function get_all_admin_emails()
	{
		$this->db->select('user_id, user_role_id, email');
