...
$(function() {
    ...
    $('#calendar_hr').fullCalendar({
            ...
            element.click(function() {
                if (event.unq == 1) {
                    $.ajax({
                        ...
                        type: "GET",
                        data: 'jd=1&is_ajax=1&mode=modal&data=view_holiday&holiday_id=' +
                            event.holiday_id,
                    ...
                    $.ajax({
                        ...
                        type: "GET",
                        data: 'jd=1&is_ajax=1&mode=modal&data=view_leave&leave_id=' +
                            event.leave_id,
                        success: function(response) {
                            if (response) {
                                $('.payroll_template_modal').modal('show')
                    ...
                    $.ajax({
                        ...
                        type: "GET",
                        data: 'jd=1&is_ajax=1&mode=view_modal&data=view_travel&travel_id=' +
                            event.travel_id,
                        success: function(response) {
                            if (response) {
                                $('.payroll_template_modal').modal('show')
                    ...
                    $.ajax({
                        ...
                        type: "GET",
                        data: 'jd=1&is_ajax=1&mode=view_modal&data=view_training&training_id=' +
                            event.training_id,
                        success: function(response) {
                            if (response) {
                                $('.payroll_template_modal').modal('show')
                    ...
                    $.ajax({
                        ...
                        type: "GET",
                        data: 'jd=1&is_ajax=1&mode=view_modal&data=view_project&project_id=' +
                            event.project_id,
                        success: function(response) {
                            if (response) {
                                $('.payroll_template_modal').modal('show')
                    ...
                    $.ajax({
                        ...
                        type: "GET",
                        data: 'jd=1&is_ajax=1&mode=view_modal&data=view_task&task_id=' +
                            event.task_id,
                        success: function(response) {
                            if (response) {
                                $('.payroll_template_modal').modal('show')
                    ...
                    $.ajax({
                        ...
                        type: "GET",
                        data: 'jd=1&is_ajax=1&mode=modal&data=view_event&event_id=' +
                            event.event_id,
                        success: function(response) {
                            if (response) {
                                $('.payroll_template_modal').modal('show')
                    ...
                    $.ajax({
                        ...
                        type: "GET",
                        data: 'jd=1&is_ajax=1&mode=modal&data=view_meeting&meeting_id=' +
                            event.meeting_id,
                        success: function(response) {
                            if (response) {
                                $('.payroll_template_modal').modal('show');
                    ...
                    $.ajax({
                        ...
                        type: "GET",
                        data: 'jd=1&is_ajax=1&mode=modal&data=view_tracking&tracking_id=' +
                            event.tracking_id,
                        success: function(response) {
                            if (response) {
                                $('.payroll_template_modal').modal('show');
        ...
        googleCalendarApiKey: 'AIzaSyCeF-cb9hsg3WtRIg55G3FqtsTflT9tYVs',
            ...
            },
            {
                events: [
                    ...
                    <?php foreach($all_holidays->result() as $holiday):?> {
                        ...
                        holiday_id: '<?php echo $holiday->holiday_id?>',
                        title: "<?php echo $holiday->event_name;?>",
                        start: '<?php echo date("Y-m-d", strtotime($holiday->start_date))?>',
                        end: '<?php echo date("Y-m-d", strtotime($holiday->end_date))?>',
                        color: '#00a65a !important',
                        unq: '1',
                    },
                    <?php endforeach;?>
                    ...
                    <?php $leave_to_date = date('Y-m-d H:i:s', strtotime($leave->to_date . ' +1 day'));?> {
                        ...
                        leave_id: '<?php echo $leave->leave_id?>',
                        title: "<?php echo $leaveType.' '.$this->lang->line('xin_hr_calendar_lv_request_by').' '.$fName;?>",
                        start: '<?php echo date("Y-m-d", strtotime($leave->from_date))?>',
                        end: '<?php echo date("Y-m-d", strtotime($leave_to_date))?>',
                        color: '#00c0ef !important',
                        unq: '2',
                    },
                    <?php endforeach;?>
                    ...
                    <?php foreach($all_upcoming_birthday as $upc_birthday):?> {
                        ...
                        // unq: '3',
                        title: "<?php echo $upc_birthday->first_name.' '.$upc_birthday->last_name?> - <?php echo $this->lang->line('xin_hr_calendar_upc_birthday');?>",
                        start: '<?php echo date("Y-m-d", strtotime($upc_birthday->next_birthday))?>',
                        color: '#dd4b39 !important',
                        unq: '3',
                    },
                    <?php endforeach;?>
                    ...
                    <?php else:?><?php echo $eName='';?><?php endif;?> {
                        ...
                        travel_id: '<?php echo $travel_request->travel_id?>',
                        title: "<?php echo $travel_request->visit_purpose.' '.$this->lang->line('xin_hr_calendar_lv_request_by').' '.$eName;?>",
                        start: '<?php echo date("Y-m-d", strtotime($travel_request->start_date))?>',
                        end: '<?php echo date("Y-m-d", strtotime($travel_request->end_date))?>',
                        color: '#3c8dbc !important',
                        unq: '4',
                    },
                    <?php endforeach;?>
                    ...
                    <?php else:?><?php echo $itype='';?><?php endif;?> {
                        ...
                        training_id: '<?php echo $training->training_id?>',
                        title: "<?php echo $itype;?>",
                        start: '<?php echo date("Y-m-d", strtotime($training->start_date))?>',
                        end: '<?php echo date("Y-m-d", strtotime($training->finish_date))?>',
                        color: '#f39c12 !important',
                        unq: '5',
                    },
                    <?php endforeach;?>
                    ...
                    <?php foreach($all_projects->result() as $projects):?> {
                        ...
                        project_id: '<?php echo $projects->project_id;?>',
                        title: "<?php echo $projects->title;?>",
                        start: '<?php echo date("Y-m-d", strtotime($projects->start_date))?>',
                        end: '<?php echo date("Y-m-d", strtotime($projects->end_date))?>',
                        color: '#605ca8 !important',
                        unq: '6',
                    },
                    <?php endforeach;?>
                    ...
                    <?php
                        ...
                        task_id: '<?php echo $tasks->task_id;?>',
                        title: "<?php echo $tname;?>",
                        start: '<?php echo date("Y-m-d", strtotime($tasks->start_date))?>',
                        end: '<?php echo date("Y-m-d", strtotime($tasks->end_date))?>',
                        color: '#d81b60 !important',
                        unq: '7',
                    },
                    <?php endforeach;?>
                    ...
                    <?php foreach($all_events->result() as $events):?> {
                        ...
                        event_id: '<?php echo $events->event_id?>',
                        title: "<?php echo $events->event_title?>",
                        start: '<?php echo date("Y-m-d", strtotime($events->event_date))?>T<?php echo $events->event_time?>',
                        color: '#001f3f !important',
                        unq: '8',
                    },
                    <?php endforeach;?>
                    ...
                    <?php foreach($all_meetings->result() as $meetings):?> {
                        ...
                        meeting_id: '<?php echo $meetings->meeting_id?>',
                        title: "<?php echo $meetings->meeting_title?>",
                        start: '<?php echo date("Y-m-d", strtotime($meetings->meeting_date))?>T<?php echo $meetings->meeting_time?>',
                        color: '#39cccc !important',
                        unq: '9',
                        className: "regular"
                    },
                    ...
                    <?php foreach($all_goals->result() as $goals):?> {
                        ...
                        tracking_id: '<?php echo $goals->tracking_id?>',
                        title: "<?php echo $goals->subject?>",
                        start: '<?php echo date("Y-m-d", strtotime($goals->start_date))?>',
                        end: '<?php echo date("Y-m-d", strtotime($goals->end_date))?>',
                        color: '#3c8dbc !important',
                        unq: '10',
                        participant: ['uploads/profile/default_female.jpg',
                            'uploads/profile/default_male.jpg'
        ...
        drop: function(date, allDay) { // this function is called when something is dropped
            ...
            $.ajax({
                ...
                type: "GET",
                data: 'jd=1&is_ajax=1&mode=modal&data=event&event_date=' + ex_date +
                    "&record=" + this_record,
                success: function(response) {
                    if (response) {
