...
if (! function_exists('get_employee_leave_category')) {
	...
	{
		$CI = &get_instance();
		if ($id_nums > 0 && isset($id_nums)) {
			$sql = "select e.leave_categories,e.user_id,l.leave_type_id,l.type_name,l.days_per_year from xin_employees as e, xin_leave_type as l where l.leave_type_id IN ($id_nums) and e.user_id = $employee_id";
		} else {
			$sql = "select e.leave_categories,e.user_id,l.leave_type_id,l.type_name,l.days_per_year from xin_employees as e, xin_leave_type as l where  e.user_id = $employee_id";
		}
...
if (!function_exists('get_allowances_type_heler')) {
	function get_allowances_type_heler(int $id, string $month_year, $allname)
	{
		$month_year = '01-' . $month_year;
		$dateTime = DateTime::createFromFormat('d-m-Y', $month_year);
		$newDate = $dateTime->format('Y-m-d');
		$CI = &get_instance();
		$CI->db->select('sa.allowance_id, sa.allowance_title, sa.allowance_amount, sa.salary_month, dt.id, dt.cpf, dt.tax, dt.sdl, dt.shg,SUM(sa.allowance_amount) as total_allowance');
		$CI->db->from('xin_salary_allowances as sa');
		$CI->db->join('xin_payment_deduction_types as dt', 'sa.payment_type_id = dt.id');
		$CI->db->where('sa.employee_id', $id);
		$CI->db->where('sa.allowance_title', $allname);
		$CI->db->where('(sa.salary_month = \'' . $newDate . '\' OR sa.salary_month IS NULL)', NULL, FALSE);
		$result = $CI->db->get();
		if ($result->num_rows() > 0) {

			return $result->row()->allowance_amount;
		} else {
			return false;
		}
	}
...
if (!function_exists('get_deduction_type_heler')) {
	function get_deduction_type_heler(int $id, $date, $type)
	{
		$CI = &get_instance();

		$CI->db->select('SUM(dt_data.amount) as deductin_amount');
		$CI->db->from('xin_deduction_type as de_t');
		$CI->db->join('xin_salary_deductions as dt_data', 'dt_data.deduction_type_id = de_t.deduction_type_id');
		$CI->db->where('dt_data.employee_id', $id);
		$CI->db->where('de_t.deduction_type', $type);
		$CI->db->where("'$date' BETWEEN from_date AND to_date");

		$result = $CI->db->get();
		if ($result->num_rows() > 0) {

			return $result->row()->deductin_amount;
		} else {
			return false;
		}
	}
}
if (!function_exists('get_commission_type_heler')) {
	function get_commission_type_heler(int $id, $date, $type)
	{
		$CI = &get_instance();
		...
		$CI->db->select('SUM(sc.commission_amount) as total_commission_amount, sc.commission_type, dt.pd_type, dt.cpf, dt.tax, dt.sdl, dt.shg');
		$CI->db->from('xin_salary_commissions as sc');
		$CI->db->join('xin_payment_deduction_types as dt', 'sc.commission_type = dt.id');
		$CI->db->where('sc.employee_id', $id);
		$CI->db->where('dt.payment_deduction_name', $type);
		$CI->db->where("MONTH(STR_TO_DATE(sc.commission_date, '%d-%m-%Y')) = MONTH(STR_TO_DATE('$month_year', '%d-%m-%Y'))", NULL, FALSE);
		$CI->db->where("YEAR(STR_TO_DATE(sc.commission_date, '%d-%m-%Y')) = YEAR(STR_TO_DATE('$month_year', '%d-%m-%Y'))", NULL, FALSE);

		$result = $CI->db->get();
		// print_r($result->result());die;
		if ($result->num_rows() > 0) {
			return $result->row()->total_commission_amount;
		} else {
			return false;
		}
	}
