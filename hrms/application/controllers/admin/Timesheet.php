...
class Timesheet extends MY_Controller
{
	...
	public function get_company_locations()
	{
		...
		if (is_numeric($keywords[0])) {
			...
			if (!empty($session)) {
				...
				$this->load->view("admin/timesheet/get_company_locations", $data);
			} else {
				redirect('admin/');
			}
		}
	...
	public function leave()
	{
		...
		// echo '<pre>'; print_r($data['all_leave_types']); exit;
		// reports to 
	...
	public function leave_details()
	{
		...
		$employee_leaves = $this->Timesheet_model->getEmployeeLeave($result[0]->employee_id);
		if ($employee_leaves) {
	...
	public function attendance_list()
	{
		...
		if ($user_info[0]->user_role_id == 1) {
			...
			// if ($ref_location_id == 0 && $project_id == 0) {
			// 	$employee = $this->Employees_model->get_attendance_employees();
			// } else if ($ref_location_id != 0 && $project_id == 0) {
			// 	$employee = $this->Employees_model->get_attendance_location_employees($ref_location_id);
			// } else if ($ref_location_id == 0 && $project_id != 0) {

			// 	$employee = $this->Employees_model->get_attendance_location_projects_employees($ref_location_id, $project_id);
			// } else {
			...
			// 	$employee = $this->Employees_model->get_attendance_location_employees($ref_location_id);
			// }
			if (empty($company_id) && empty($ref_location_id)) {
				$employee = $this->Employees_model->get_attendance_employees();
			} else if ($company_id != 0 && empty($ref_location_id)) {
				$employee = $this->Employees_model->get_attendance_employees_company($company_id);
			} else {
				$employee = $this->Employees_model->get_attendance_employees_company_location($company_id, $ref_location_id);
			}
		} else {
			...
			if ($r->user_role_id != 1) {
				...
				if ($check->num_rows() > 0) {
					...
					foreach ($total_hrs->result() as $hour_work) {
						...
						// echo gettype($hrs_old_seconds);
						// $final_total = $hrs_old_seconds + $final_total;
					}
					if ($Total == '') {
						$total_work = '00:00';
						// $final_total = 0;
						...
						$total_work = $Total;
						// $final_total = gmdate("H:i:s", $final_total);
					}
				...
				if ($check_out->num_rows() == 1) {
					...
					$leave_date = $this->Timesheet_model->leave_date($r->user_id, $attendance_date);
					if (isset($leave_date) && count($leave_date) > 0) {
						$half_day = $leave_date[0]->is_half_day;
					} else {
						$half_day = '';
					}
					if ($first_out[0]->clock_out != '') {
						...
						if ($early_new_time <= $clock_out_time_new || $half_day) {
							$total_time_e = '00:00';
						} else {
				...
				if ($user_info[0]->user_role_id == 1) {
					$fclckIn = $clkInIp;
					if ($in_lat != '' && $in_long != '') {
						$fclcInLocation = '<a href="https://www.google.com/maps/search/?api=1&query=' . $in_lat . ',' . $in_long . '">Clock In Location</a>';
					} else {
						$fclcInLocation = '-';
					}
					//clock out location
					if ($out_lat != '' && $out_long != '') {
						$fclcOutLocation = '<a href="https://www.google.com/maps/search/?api=1&query=' . $out_lat . ',' . $out_long . '">Clock Out Location</a>';
					} else {
						$fclcOutLocation = '-';
					}
					$fclckOut = $clkOutIp;
				...
				$data[] = array(
					...
					$fclckIn,
					// $in_lat,
					!empty($in_lat) ? "<a target='_blank' href='https://www.google.com/maps?q={$in_lat},{$in_long}'>Clock in Location</a>" : '',
					// $in_long,
					$fclckOut,
					!empty($out_lat) ? "<a target='_blank' href='https://www.google.com/maps?q={$out_lat},{$out_long}'>Clock Out Location</a>" : '',
					// $out_lat,
					// $out_long,
					$total_time_l,
					$total_time_e,
	...
	public function project_attendance_list()
	{
	...
	public function leave_list()
	{
		...
		if ($this->input->get("ihr") == 'true') {
			if ($this->input->get("company_id") == 0 && $this->input->get("employee_id") == 0 && $this->input->get("status") == 0) {
				$leave = $this->Timesheet_model->get_leaves($this->input->get("from_date"), $this->input->get("to_date"));
			} else if ($this->input->get("company_id") != 0 && $this->input->get("employee_id") == 0 && $this->input->get("status") == 0) {
				$leave = $this->Timesheet_model->filter_company_leaves($this->input->get("company_id"), $this->input->get("from_date"), $this->input->get("to_date"));
			} else if ($this->input->get("company_id") != 0 && $this->input->get("employee_id") != 0 && $this->input->get("status") == 0) {
				$leave = $this->Timesheet_model->filter_company_employees_leaves($this->input->get("company_id"), $this->input->get("employee_id"), $this->input->get("from_date"), $this->input->get("to_date"));
			} else if ($this->input->get("company_id") != 0 && $this->input->get("employee_id") != 0 && $this->input->get("status") != 0) {
				$leave = $this->Timesheet_model->filter_company_employees_status_leaves($this->input->get("company_id"), $this->input->get("status"), $this->input->get("from_date"), $this->input->get("to_date"));
			} else if ($this->input->get("company_id") != 0 && $this->input->get("employee_id") == 0 && $this->input->get("status") != 0) {
				$leave = $this->Timesheet_model->filter_company_only_status_leaves($this->input->get("company_id"), $this->input->get("status"), $this->input->get("from_date"), $this->input->get("to_date"));
			}
		} else {
	...
	public function add_leave()
	{
		if ($this->input->post('add_type') == 'leave') {
			...
			$date2 = date_create(date('Y-m-d', strtotime($joining_date)));
			$diff = date_diff($date1, $date2);
			//echo "<pre>";print_r($diff->m);exit;

			$end_store_date = $this->input->post('end_date');
			...
			if ($datetime1->format('Y') != $datetime2->format('Y')) {
				$Return['error'] = 'Can\'t apply between dates of two calendar year' . $datetime1->format('Y');
			}
			$interval = new DateInterval('P1D');
			$period = new DatePeriod($datetime1, $interval, $datetime2);
			...
			if ($start_date != '') {
				...
				if ($this->input->post('leave_type') != 1) {
					...
					// $type = $this->Timesheet_model->getEmployeeLeaveCount($this->input->post('leave_type'), $this->input->post('employee_id'));
					// if($type) {
					...
					$applied_leaves = $this->Timesheet_model->getEmployeeAppliedLeaveCount($leave_type, date('Y'), $employee_id);
					...
					$type = $this->Timesheet_model->getEmployeeLeaveCount($leave_type, $employee_id);

					if ($type) {
						$leave_diff = new DatePeriod($datetime1, $interval, $datetime2);


						$id = $type->id;
						$type_name = $type->type_name;
						// $total = $type->no_of_leaves;
						$total = $type->balance_leave_check;
						$leave_remaining_total = (float)$total - (float)$leaves_taken;

						// $carried_leave = $type->carried_leave;
						// $leave_without_carried = (float)$leave_remaining_total - (float)$carried_leave;
						// $leave=0;


						// check for error message
						$previous_year = $this->Employees_model->getEmployeeLeaveCountForLeave($leave_type, date('Y') - 1, $employee_id);
						$current_year = $this->Employees_model->getEmployeeLeaveCountForLeave($leave_type, date('Y'), $employee_id);
						$setting = $this->Xin_model->read_setting_info(1);
						if ($setting[0]->module_prorated_leave == 'yes' && $leave_type == 22) {
							$type_name = $type->type_name;
						...
						// end check for error message
						$previous_year = $this->Employees_model->getEmployeeLeaveCountForLeave($leave_type, date('Y') - 1, $employee_id);
						$current_year = $this->Employees_model->getEmployeeLeaveCountForLeave($leave_type, date('Y'), $employee_id);
						$previous_year_condition =  isset($previous_year->remain_leave) ? $previous_year->remain_leave : 0;

						if ($leave_type == 22 && ($remaining_leave < $no_of_days && $remaining_leave > $current_year->balance_leave + $previous_year_condition)) {
							$Return['error'] = "You Can Not Take More Then Available Leave!";
						}
					...
					// if ($carried_leave < $leaves_taken ) {					
					// 		$remaining_carried_leave =  $carried_leave - count($leave_period);
					// 		$l_data = array('carried_leave' => $remaining_carried_leave);
					// 		$this->Timesheet_model->update_carried_leave($l_data);

					// }
					// $setting = $this->Xin_model->read_setting_info(1);
					// if ($setting[0]->module_prorated_leave == 'yes') {
					// 		if($type_name == "Annual Leave"){
					// 		$leave_d = array();
					// 		foreach ($leave_diff as $ld) {
					// 			$pd_day = $ld->format('l');
					// 			if ($pd_day == 'Monday') {
					// 				if ($office_shift[0]->monday_in_time != '') {
					// 					$leave_d[] = $ld->format('Y-m-d');
					// 				}
					// 			} else if ($pd_day == 'Tuesday') {
					// 				if ($office_shift[0]->tuesday_in_time != '') {
					// 					$leave_d[] = $ld->format('Y-m-d');
					// 				}
					// 			} else if ($pd_day == 'Wednesday') {
					// 				if ($office_shift[0]->wednesday_in_time != '') {
					// 					$leave_d[] = $ld->format('Y-m-d');
					// 				}
					// 			} else if ($pd_day == 'Thursday') {
					// 				if ($office_shift[0]->thursday_in_time != '') {
					// 					$leave_d[] = $ld->format('Y-m-d');
					// 				}
					// 			} else if ($pd_day == 'Friday') {
					// 				if ($office_shift[0]->friday_in_time != '') {
					// 					$leave_d[] = $ld->format('Y-m-d');
					// 				}
					// 			} else if ($pd_day == 'Saturday') {
					// 				if ($office_shift[0]->saturday_in_time != '') {
					// 					$leave_d[] = $ld->format('Y-m-d');
					// 				}
					// 			} else if ($pd_day == 'Sunday') {
					// 				if ($office_shift[0]->sunday_in_time != '') {
					// 					$leave_d[] = $ld->format('Y-m-d');
					// 				}
					// 			}
					// 		}
					// 			$no_days= date('z')+1;
					// 		$leave = (float)$leave_remaining_total / 365 * $no_days;
					// 		if ($no_of_days > $leave) {
					// 			$Return['error'] = "You Can take only " . round($leave);
					// 		}
					// 		if ($Return['error'] == '') {
					// 			$leave_remaining_total = (float)$leave_remaining_total - (float)$leave;
					// 		}
					// 	}
					// }
			...
			if($leave_remaining_total == 0){
				...
				if ($result == TRUE) {
					...
					// get leave type
					$leave_type = $this->Timesheet_model->getEmployeeLeaveCount($row->leave_type_id, $this->input->post('employee_id'));
					if ($leave_type) {
					...
					if ($setting[0]->enable_email_notification == 'yes') {
						...
						$admin_email_ids =  [];
						$admin_emails = $this->Xin_model->get_all_admin_emails();
						if ($admin_emails) {
							foreach ($admin_emails as $ae) {
								($ae->email) ? $admin_email_ids[] = $ae->email : '';
							}
						}
						...
						$message = '
			<div style="background:#f6f6f6;font-family:Verdana,Arial,Helvetica,sans-serif;font-size:12px;margin:0;padding:0;padding: 20px;">
			<img src="' . $logo . '" title="' . $cinfo[0]->company_name . '"><br>' . str_replace(array("{var site_name}", "{var site_url}", "{var employee_name}" , "{var leave_start_date}" , "{var leave_end_date}"), array($cinfo[0]->company_name, site_url(), $full_name,$this->input->post('start_date'),$end_store_date), htmlspecialchars_decode(stripslashes($template[0]->message))) . '</div>';
						if (!is_null($reports_to)) {
							hrsale_mail($user_info[0]->email, $full_name, $reports_to[0]->email,  $subject, $message, $admin_email_ids);
						} else {
							hrsale_mail($user_info[0]->email, $full_name, $cinfo[0]->email, $subject, $message, $admin_email_ids);
						}
					}
				} else {
					$Return['error'] = $this->lang->line('xin_error_msg');
	...
	public function add_attendance()
	{
		...
		if ($this->input->post('add_type') == 'attendance') {
			...
			// $attendance_date = $this->Xin_model->set_date_format($this->input->post('attendance_date_m'));
			...
			if ($this->input->post('add_type') == 'attendance') {
				...
				// $date /d= date('d-m-Y H:i:s');
				$curtime = $attendance_date . ' ' . $clock_in . ':00';

				$query = $this->Timesheet_model->check_admin_user_attendance($this->input->post('employee_id_m'), $attendance_date);
				$result = $query->result();
				if ($query->num_rows() < 1) {
					$total_rest = '';
				} else {
	...
	public function edit_leave()
	{
		...
		if ($this->input->post('edit_type') == 'leave') {
			...
			if ($start_date != '') {
				...
				// $result = $this->Timesheet_model->add_leave_record($data);
				$result = $this->Timesheet_model->update_leave_record($data, $update_id);
	...
	public function update_leave_status()
	{
		...
		if ($this->input->post('update_type') == 'leave') {
			...
			$check_prev = $this->Employees_model->getEmployeeLeaveCountForLeave($this->input->post('leave_type'), date('Y') - 1, $this->input->post('employee_id'));

			$check_current = $this->Employees_model->getEmployeeLeaveCountForLeave($this->input->post('leave_type'), date('Y'), $this->input->post('employee_id'));
			$no_of_leave = $this->input->post('no_of_leave');
			$check = $this->Employees_model->getLeaveDetailsById($id);

			// leave taken count
			$user = $this->Xin_model->read_user_info($this->input->post('employee_id'));
			$office_shift = $this->Timesheet_model->read_office_shift_information($user[0]->office_shift_id);
			$employee_leaves = $this->Timesheet_model->getEmployeeLeave($this->input->post('employee_id'));
			$leaves_taken_count = 0;
			if ($employee_leaves) {

				foreach ($employee_leaves as $v) {
					$remaining_leaves = $this->Timesheet_model->employeeYearLeaveCountBeforeStatusChange($v->leave_type_id, $v->leave_year, $v->employee_id, $id);
					$v->remaining_leaves = $remaining_leaves;
					$leaves_taken = $v->remaining_leaves;
			...
			// end lave taken count
			$no_of_leave = $leaves_taken_count;
			if ($this->input->post('status') == 2 && $check->status != 2 && $this->input->post('leave_type') == 22) {

				// if($check_prev->balance_leave != 0){
				$reman = 0;
				if ($check_prev->remain_leave != null && isset($check_prev->remain_leave) >= $no_of_leave) {
					$l_data = array(
						// 'balance_leave'	=>	$check_prev->remain_leave - $no_of_leave,
						'remain_leave'	=> $check_prev->remain_leave - $no_of_leave,
						// 'carried_leave' => 	$check_prev->remain_leave - $no_of_leave
						// 1

					);
					$this->Timesheet_model->update_carried_leave($l_data, date('Y') - 1, $this->input->post('employee_id'), $this->input->post('leave_type'));
				} else {
					$prev_remain_leave = isset($check_prev->remain_leave) && $check_prev->remain_leave != null ? $check_prev->remain_leave : 0;
					$reman = $no_of_leave - $prev_remain_leave;
					$l_data = array(
						// 'balance_leave'	=>	0,
						// 'carried_leave' => 	0
						'remain_leave'	=> 0,
				...
				if ($reman != 0) {
					$l_data = array(
						'balance_leave'	=>	$check_current->balance_leave - $reman,
						// 'carried_leave' => 	$check_current->carried_leave - $no_of_leave
						'remain_leave'	=> $check_current->balance_leave - $reman,
						// 3
					);
					$this->Timesheet_model->update_carried_leave($l_data, date('Y'), $this->input->post('employee_id'), $this->input->post('leave_type'));
				} else {
					$l_data = array(
						'balance_leave'	=>	$check_current->balance_leave - $no_of_leave,
						// 'carried_leave' => 	$check_current->carried_leave - $no_of_leave
						'remain_leave'	=> $check_current->balance_leave - $no_of_leave,
						// 4
			...
			if ($result == TRUE) {
				...
				if ($setting[0]->enable_email_notification == 'yes') {
					...
					if ($this->input->post('status') == 2) {
						$type = $this->Timesheet_model->getEmployeeLeaveCount($leave_type, $employee_id);
						$leave_remaining_total = $type->balance_leave - $no_of_leave;
						$carried_leave = $type->carried_leave;
						// $leave_without_carried = $leave_remaining_total - $carried_leave;
						// if ($carried_leave < $leave_remaining_total ) {

						// 			$remaining_carried_leave =  $carried_leave - $no_of_leave;
						// 			$l_data = array(
						// 				'balance_leave'=>$leave_remaining_total,
						// 				'carried_leave' => $remaining_carried_leave);
						// 			$this->Timesheet_model->update_carried_leave($l_data);
	...
	{
		$cinfo = $this->Xin_model->read_company_setting_info(1);
		// hrsale_mail($cinfo[0]->email, $cinfo[0]->company_name, "debasissahoochinku@gmail.com", "Test", '$message');
		...
		try {
			...
			$mail->SMTPAuth   = true;
			$mail->Username   = 'hr@hsglobalm.co'; // Your Office365 Email
			$mail->Password   = 'zqtmmmhllnwwjbhq'; // Use App Password, NOT regular password
			$mail->SMTPSecure = 'tls';
			$mail->Port       = 587;
			$mail->SMTPDebug = 2; 

			// Email Content
			$mail->setFrom('hr@hsglobalm.co', 'HR Department');
			$mail->addAddress('debasissahoochinku@gmail.com', 'John Doe');
			$mail->Subject = 'Test Email via Office 365 SMTP';
			$mail->Body    = 'This is a test email sent using Office 365 SMTP in PHP!';
