...
class Payroll extends MY_Controller
{
	...
	public function generate_payslip()
	{
		...
		if ($this->input->get('ismobile') == 'true') {
			$session 			= 	$this->session->userdata('username');
			$data['title'] 		= 	"PAYROLL" . ' | ' . $this->Xin_model->site_title();
			$result 			= 	$this->Employees_model->read_employee_information($session['user_id']);
			$data['name'] 		= 	$result[0]->first_name . " " . $result[0]->last_name;
			$data['main_data'] 	= 	$this->db->select('*')->from('xin_salary_payslips')->where('employee_id', $session['user_id'])->get()->result();
	...
	public function add_pay_to_all_by_id()
	{
		...
		if ($this->input->post('add_type') == 'payroll') {
			foreach ($this->input->post('id') as $idd) {


				$result = $this->db->where('user_id', $idd['id'])->get('xin_employees')->result();
				...
				foreach ($result as $empid) {
					...
					// print_r($basic_salary);
					// $pay_basic = $this->Employees_model->get_basic_pay($user_id, $p_date);
					// $pay = 0;
					// if ($idd['basic_salary'] == "true") {
					...
					if ($basic_salary > 0) {
						...
						// departmentpayslip_list_bulk_rigo
						...
						if ($salary_allowances && $idd['allowance_amount'] == "true") {
							foreach ($salary_allowances as $sa) {
								if ($system[0]->is_half_monthly == 1) {
									if ($system[0]->half_deduct_month == 2) {
										$eallowance_amount = $sa->allowance_amount / 2;
						...
						if ($salary_allowances && $idd['allowance_amount'] == "false") {
							foreach ($salary_allowances as $sa) {
								if ($system[0]->is_half_monthly == 1) {
									if ($system[0]->half_deduct_month == 2) {
										$eallowance_amount = $sa->allowance_amount / 2;
						...
						$gross_allowance_amount = $allowance_amount == 0 ? $allowance_amount_B : $allowance_amount;
						...
						$General_amount_B = 0;
						// $appeacial_allowance = $this->Employees_model->get_appeacial_allowance($user_id, $p_date);
						// if ($idd['motivation_amount'] == "true") {

						// 	$motivation_amount = $appeacial_allowance['motivation_amount'];
						// }
						// if ($idd['General_amount'] == "true") {


						// 	$General_amount = $appeacial_allowance['General_amount'];
						// }
						// if ($idd['General_amount'] == "false") {


						// 	$General_amount_B = $appeacial_allowance['General_amount'];
						// }
						// if ($idd['motivation_amount'] == "false") {
						...
						if ($unpaid_leaves && $idd['unpaid_leave_amount'] == "true") {
							foreach ($unpaid_leaves as $k => $l) {
						...
						if ($month_date_join == $p_date) {
							...
							foreach ($period as $p) {
								...
								$is_holiday = $this->Timesheet_model->is_holiday_on_date($user[0]->company_id, $m_p_date);
								if ($is_holiday) {
						...
						$gross_pay = round((($basic_salary + $gross_allowance_amount) / $no_of_working_days) * $no_of_days_worked, 2);
						$unpaid_leave_amount = ($basic_salary + $gross_allowance_amount) - $gross_pay;
						if ($idd['unpaid_leave_amount'] == "true") {
							$unpaid_leave_amount = $unpaid_leave_amount;
						} elseif ($idd['unpaid_leave_amount'] == "false") {
							$unpaid_leave_amount = 0;
							$unpaid_leave_amount_B = $unpaid_leave_amount;
						}
						...
						$loan_de_amount = 0;
						$loan_de_amount_B = 0;
						if ($count_loan_deduction > 0 && $idd['loan_de_amount'] == "true") {
							foreach ($salary_loan_deduction as $sl_salary_loan_deduction) {
								$loan_de_amount += $sl_salary_loan_deduction->loan_deduction_amount;
							}
						} else {
						...
						if ($count_loan_deduction > 0 && $idd['loan_de_amount'] == "false") {
							foreach ($salary_loan_deduction as $sl_salary_loan_deduction) {
								$loan_de_amount_B += $sl_salary_loan_deduction->loan_deduction_amount;
							}
						} else {
						...
						$other_payments_amount = 0;
						$other_payments_amount_B = 0;
						if (!is_null($other_payments) && $idd['other_payments_amount'] == "true") :
							foreach ($other_payments->result() as $sl_other_payments) {
								// $other_payments_amount += $sl_other_payments->payments_amount;

								if ($sl_other_payments->ad_hoc_allowance == "Ad Hoc") {
									if ($p_date == date('m-Y', strtotime($sl_other_payments->date))) {
										if ($system[0]->is_half_monthly == 1) {
											if ($system[0]->half_deduct_month == 2) {
												$epayments_amount = $sl_other_payments->payments_amount / 2;
											} else {
										...
										if ($sl_other_payments->cpf_applicable == 1) {
											$g_ordinary_wage += $epayments_amount;
										}
						...
						if (!is_null($other_payments) && $idd['other_payments_amount'] == "false") :
							foreach ($other_payments->result() as $sl_other_payments) {
								// $other_payments_amount_B += $sl_other_payments->payments_amount;
						...
						if ($commissions && $idd['commission'] == "false") {
							foreach ($commissions as $c) {
								if ($system[0]->is_half_monthly == 1) {
									if ($system[0]->half_deduct_month == 2) {
										$ecommissions_amount = $c->commission_amount / 2;
								...
								$commissions_amount_B += $ecommissions_amount;
							}
						}

						//share options
						$share_options_amount = 0;
						$share_options_amount_B = 0;
						$share_options = $this->Employees_model->getEmployeeShareOptions($user_id, $p_date);
						if ($share_options && $idd['share_options_amount'] == "true") {
							$eebr_amount = 0;
							$eris_amount = 0;
							foreach ($share_options as $s) {
								$scheme = $s->so_scheme;
						...
						if ($share_options && $idd['share_options_amount'] == "false") {
							$eebr_amount = 0;
							$eris_amount = 0;
							foreach ($share_options as $s) {
								$scheme = $s->so_scheme;
							...
							$share_options_amount_B = round($eebr_amount + $eris_amount, 2);
							// $g_additional_wage += $share_options_amount;
							// $g_sdl += $share_options_amount;
							// $g_shg += $share_options_amount;
						}
						...
						$overtime_count = $this->Overtime_request_model->get_overtime_request_count($user_id, $p_date);
						$re_hrs_old_int1 = 0;
						$re_hrs_old_seconds = 0;
						$re_pcount = 0;
						foreach ($overtime_count as $overtime_hr) {
						...
						$result = $this->Payroll_model->total_hours_worked($user_id, $p_date);
						...
						if ($idd['basic_salary'] == "true") {
							if ($empid->wages_type == 1) {
								$pay = $empid->basic_salary;
							} else {
								$basic_salary = $empid->basic_salary;
								$pay = (float)$pcount * (float)$basic_salary;
							}
						}
						...
						$statutory_deductions_amount = 0;
						$statutory_deductions_amount_B = 0;

						$statutory_deductions = $this->Employees_model->set_employee_statutory_deductions($user_id);
						if (!is_null($statutory_deductions) && $idd['statutory_deductions_amount'] == "true") :
						...
						if (!is_null($statutory_deductions) && $idd['statutory_deductions_amount'] == "false") :
							foreach ($statutory_deductions->result() as $sl_statutory_deductions) {
								if ($system[0]->statutory_fixed != 'yes') :
									$sta_salary = $pay;
									$st_amount = $sta_salary / 100 * $sl_statutory_deductions->deduction_amount;
									$statutory_deductions_amount_B += $st_amount;
								else :
									$statutory_deductions_amount_B += $sl_statutory_deductions->deduction_amount;
								endif;
							}
						endif;
						...
						$all_overtime_P_B = 0;
						$restday_pay_B = 0;
						// $overt = $this->Employees_model->get_overtime($user_id, $p_date);
						if ($idd['all_overtime_P'] == "true") {

							$all_overtime_P = $overtime_amount;
							$all_overtime_t = $overtime_amount;
						}
						if ($idd['all_overtime_P'] == "false") {
							$all_overtime_P_B = $overtime_amount;
						}
						// $basic_salary = $pay;
						// $restday_pay1 = $this->Employees_model->restday_pay($user_id, $p_date);
						// if ($idd['restday_pay'] == "true") {
						// 	$restday_pay = $restday_pay1['rest_daya_amount'];
						// }
						// if ($idd['restday_pay'] == "false") {
						// 	$restday_pay_B = $restday_pay1['rest_daya_amount'];
						// }
						$total_employee_deduction = 0;
						$total_employee_deduction_B = 0;

						$employee_deduction = $this->Payroll_model->get_deduction_detail($user_id, $p_date);

						if ($idd['total_deduction'] == "true" && $employee_deduction) {
							$total_employee_deduction = array_sum(array_column($employee_deduction, 'amount'));
						}
						if ($idd['total_deduction'] == "false") {
							$total_employee_deduction_B = array_sum(array_column($employee_deduction, 'amount'));
						}
						// cpf 
						...
						// cpf 
						// $get_cliam_data = 0;
						// $LeaveEnchashment = $this->Employees_model->get_leave_Encashment($user_id, $p_date);
						// $encamoun = $LeaveEnchashment['ench_amount'];
						// $other_payments_amount += $encamoun;
						// $get_cliam_data = $this->Employees_model->get_cliam_data($user_id, $p_date);

						// add amount //basic_salary
						$add_salary = $allowance_amount + $pay  + $all_overtime_P + $other_payments_amount + $restday_pay + $commissions_amount + $share_options_amount;
						// add amount
						$total_deduction = floatval($loan_de_amount) + $statutory_deductions_amount;
						$net_salary_default = $add_salary - floatval($loan_de_amount) - $statutory_deductions_amount - $total_employee_deduction;
						...
						// $other_payments_amount_B = 0;
						// $unpaid_leave_amount_B = 0;
						if ($idd['loan_de_amount'] == "false") {
							$loan_de_amount_B = $loan_de_amount;
						}
						...
						$data_bacup = array(
							'gross_salary' => $basic_salary_B,
							'total_commissions' => $commissions_amount_B,
							'total_statutory_deductions' => $statutory_deductions_amount_B,
							'total_other_payments' => $other_payments_amount_B,
							'total_allowances' => $allowance_amount_B,
							'total_loan' => $loan_de_amount_B,
							'total_overtime' => $all_overtime_P_B,
							'employee_claim' => 0,
							'user_id' => $user_id,
							'leave_deductions' => $unpaid_leave_amount_B,
							'net_salary' => $net_salary,
							'total_share' => $share_options_amount_B,
							'additional_allowances' => $this->input->post('additional_allowances_bc'),
							'deduction_amount' => $total_employee_deduction_B,
							'General_amount' => $General_amount_B,
							'motivation_amount' => $motivation_amount_B,
							'restday_pay' => $restday_pay_B,
							'balance_amount' => $idd['balance_amount'],
						);
						...
						$data = array(
							...
							'total_overtime_amount' => $all_overtime_P,
							// 'claim_amount' => $employee_claim,
							'cpf_employee_amount' => $cpf_employee,
							'cpf_employer_amount' => $cpf_employer,
							'leave_deduction' => $unpaid_leave_amount,
							'contribution_fund' => $total_contribution,
							'share_option_amount' => $share_options_amount,
							'additonal_allowance' => $additional_allowances,
							'deduction_amount' => $total_employee_deduction,
							'hours_worked' => ($user[0]->wages_type == 2) ? $pcount : 0,
							'is_payment' => '1',
							'payment_mode' => '',
							'payslip_type' => ($user[0]->wages_type == 2) ? 'hourly' : 'full_monthly',
							'balance_amount' => $idd['balance_amount'],
							'payslip_key' => $jurl,
						...
						// print_r($data);exit;
						$result_previous = $this->Payroll_model->add_salary_payslip_get($user_id, $p_date);
						$this->db->where('user_id', $user_id)->update('xin_employees', ['payment_status' => $idd['balance_amount'], 'payment_type' => 'MO']);
						// print_r($result_previous);
						// exit;
						if ($result_previous) {
							$data_again = array(
								'employee_id' => $user_id,
								'department_id' => $result_previous->department_id,
								'company_id' => $result_previous->company_id,
								'location_id' => $result_previous->location_id,
								'designation_id' => $result_previous->designation_id,
								'salary_month' => $p_date,
								// 'basic_salary' => $basic_salary,
								'gross_salary' => $basic_salary,


								'total_commissions' => $commissions_amount + $result_previous->total_commissions,
								'total_statutory_deductions' => $statutory_deductions_amount + $result_previous->total_statutory_deductions,
								'total_other_payments' => $other_payments_amount + $result_previous->total_other_payments,
								'total_allowances' => $allowance_amount + $result_previous->total_allowances,
								'total_loan' => floatval($loan_de_amount) + $result_previous->total_loan,
								'total_overtime' => $overtime_amount + $result_previous->total_overtime,
								'total_overtime_amount' => $all_overtime_P,

								'cpf_employee_amount' =>  $cpf_employee + $result_previous->cpf_employee_amount,
								'cpf_employer_amount' => $cpf_employer + $result_previous->cpf_employer_amount,
								'leave_deduction' => $unpaid_leave_amount + $result_previous->leave_deduction,
								'contribution_fund' => $total_contribution + $result_previous->total_contribution,
								'share_option_amount' => $share_options_amount + $result_previous->share_option_amount,
								...
								'balance_amount' => $idd['balance_amount'],
								'net_salary' => $net_salary + $result_previous->net_salary,
							...
							$result = $this->Payroll_model->add_salary_payslip_update($data_again, $user_id);
						} else {
						...
						if ($result) {
							...
							// $appeacial_allowance = $this->Employees_model->get_appeacial_allowance($user_id, $this->input->post('month_year'));
							// if ($appeacial_allowance['motivation_amount'] > 0 && $idd['motivation_amount'] == "true") {
							// 	$motivation_amm = $appeacial_allowance['motivation_amount'];
							...
							// }
							// if ($appeacial_allowance['General_amount'] > 0 && $idd['General_amount'] == "true") {
							...
							if ($idd['total_deduction'] == "true" || $employee_deduction) {
								...
								$other_benefit_list = $this->PaymentDeduction_Model->get_all_employee_other_benefits_for_payslip($user_id, $p_date);
							...
							// new logic by Debasis
							$month_date_join = date('m-Y', strtotime($user[0]->date_of_joining));
							$lastday = date('t-m-Y', strtotime("01-" . $this->input->post('month_year')));
							$month_last_date = date('m-Y', strtotime($lastday));
							...
							$no_of_days_worked = 0;
							$same_month_holidays_count = 0;
							if ($month_date_join == $this->input->post('month_year')) {
								$interval = new DateInterval('P1D');
								$period = new DatePeriod($first_date, $interval, $month_end_date);
								foreach ($period as $p) {
									$p_day = $p->format('l');
							...
							if ($count_other_payment > 0 && $idd['other_payments_amount'] == "true") {
								foreach ($salary_other_payments as $sl_other_payments) {

									if ($sl_other_payments->ad_hoc_allowance == "Ad Hoc") {
										if ($this->input->post('month_year') == date('m-Y', strtotime($sl_other_payments->date))) {
											if ($system[0]->is_half_monthly == 1) {
												if ($system[0]->half_deduct_month == 2) {
													$epayments_amount = $sl_other_payments->payments_amount / 2;
												} else {
							...
							$salary_statutory_deductions = $this->Employees_model->set_employee_statutory_deductions($user_id);

							$count_statutory_deductions = $this->Employees_model->count_employee_statutory_deductions($user_id);
							$statutory_deductions_amount = 0;
							if ($count_statutory_deductions > 0 && $idd['statutory_deductions_amount'] == "true") {
								foreach ($salary_statutory_deductions->result() as $sl_statutory_deduction) {
	...
	public function payslip_list()
	{
		...
		$originalDate = '01-07-2024'; // DD-MM-YYYY format
		...
		foreach ($payslip->result() as $r) {
			...
			if (count($exit_employee) > 0) {
				...
				// $exit_date = date('m-Y', strtotime($e_date . ' + 1 months'));
				...
				if ($exit_date >= $p_date) {
					...
					if ($basic_salary == 0 || $basic_salary == '') {
						...
						$balance_row = $this->db->select('balance_amount')
							->where('employee_id', $r->user_id)
							->get('xin_salary_payslips')
							->row();

						if ($balance_row) {
							$balance_amount = $balance_row->balance_amount;
						} else {
							// Handle the case where no balance is found
							$balance_amount = 0; // Or any default value you want to assign
						}
						if ($balance_amount > 0) {
							$mpay .= '<span data-toggle="tooltip" data-placement="top" title="' . $this->lang->line('xin_payroll_make_payment') . '"><button type="button" class="btn icon-btn btn-xs btn-outline-secondary waves-effect waves-light" data-toggle="modal" data-target=".view-modal-data" data-employee_id="' . $r->user_id . '" data-payment_date="' . $p_date . '" data-company_id="' . $this->input->get("company_id") . '"><span class="fa fas fa-money"></span></button></span>';
						}
						$fmpay = $mpay;
					}
					...
					$p = $payslips->result();
					// if (count($p) > 0) {
					// 	$p_basic_salary = $this->Xin_model->currency_sign($p[0]->basic_salary, $r->user_id);
					// 	$p_cpf_employee = $this->Xin_model->currency_sign($p[0]->cpf_employee_amount, $r->user_id);
					// 	$p_net_salary = $this->Xin_model->currency_sign($p[0]->net_salary, $r->user_id);
					// } else {
					// 	$p_basic_salary = $basic_salary;
					// 	$p_cpf_employee = $cpf_employee;
					// 	$p_net_salary = $this->Xin_model->currency_sign($total_net_salary, $r->user_id);
					// }
					...
					$data[] = array(
						...
						$emp_payroll_wage,
						$basic_salary,
						// $this->Xin_model->currency_sign($p_cpf_employee, $r->user_id),
				...
				$appeacial_allowance['General_amount'] = 0;
				$appeacial_allowance['motivation_amount'] = 0;
				...
				$g_sdl += $basic_salary;
				// 2: all allowances
				$g_ordinary_wage += floatval($appeacial_allowance['General_amount']);
				$g_shg += floatval($appeacial_allowance['General_amount']);
				$g_sdl += floatval($appeacial_allowance['General_amount']);

				// 2: all allowances

				// 2: all allowances
				$g_ordinary_wage += floatval($appeacial_allowance['motivation_amount']);
				$g_shg += floatval($appeacial_allowance['motivation_amount']);
				$g_sdl += floatval($appeacial_allowance['motivation_amount']);
				...
				if ($system[0]->is_half_monthly == 1) {
					...
					if ($payment_check->num_rows() > 0) {
						$make_payment = $this->Payroll_model->read_make_payment_payslip($r->user_id, $p_date);
						$view_url = site_url() . 'admin/payroll/payslip/id/' . $make_payment[0]->payslip_key;
						if ($make_payment[0]->status == 3) {
							$status = '<span class="label label-warning">' . "Partially" . '</span>';
						} else if ($make_payment[0]->status == 0) {
				...
				// add amount
				$total_earning = floatval($appeacial_allowance['General_amount']) +  floatval($appeacial_allowance['motivation_amount']) + $basic_salary + $allowance_amount + $restday_pay + $overtime_amount + $commissions_amount + $other_payments_amount + $share_options_amount;
				$total_deduction = floatval($loan_de_amount) + $statutory_deductions_amount;
				$total_net_salary = $total_earning - $total_deduction - $unpaid_leave_amount - $deduction_amount;
				...
				//action link
				if ($r->payment_status > 0 && $r->payment_type == 'MO') {
					$mp = '<span data-toggle="tooltip" data-placement="top" title="' . $this->lang->line('xin_payroll_make_payment') . '"><button type="button" class="btn icon-btn btn-xs btn-outline-secondary waves-effect waves-light" data-toggle="modal" data-target=".emo_monthly_pay_again" data-employee_id="' . $r->user_id . '" data-payment_date="' . $p_date . '" data-company_id="' . $this->input->get("company_id") . '"><span class="fa fas fa-money"></span></button></span>';
					$act = $detail . $mp . $fmpay . $delete;
				} elseif ($r->payment_status > 0 && $r->payment_type == 'HO') {
					$mp = '<span data-toggle="tooltip" data-placement="top" title="' . $this->lang->line('xin_payroll_make_payment') . '"><button type="button" class="btn icon-btn btn-xs btn-outline-secondary waves-effect waves-light" data-toggle="modal" data-target=".emo_hourly_pay_again" data-employee_id="' . $r->user_id . '" data-payment_date="' . $p_date . '" data-company_id="' . $this->input->get("company_id") . '"><span class="fa fas fa-money"></span></button></span>';
					$act = $detail . $mp . $fmpay . $delete;
				} else {
				...
				$mode_of_payment = 0;
				// if (count($p) > 0) {
				// 	$basic_salary = $this->Xin_model->currency_sign($p[0]->basic_salary, $r->user_id);
				// 	$p_cpf_employee = $this->Xin_model->currency_sign($p[0]->cpf_employee_amount, $r->user_id);
				// 	$p_net_salary = $this->Xin_model->currency_sign($p[0]->net_salary, $r->user_id);
				// } else {
				// 	$basic_salary = $basic_salary;
	...
	public function get_salrry_status($user_id, $type)
	{

		$employee_pending_salary = $this->Employees_model->get_pendding_salary($user_id, '08-2024');
		if ($employee_pending_salary) {
			// if ($employee_pending_salary->$type > 0) {
			if ($type > 0) {
				return '';
			} else {
	...
	public function get_salrry_status_class($user_id, $type)
	{
		...
		if ($employee_pending_salary) {
			...
			if ($type > 0) {
				...
				return 'column_checkbox_paid';
			}
		} else {
			return 'column_checkbox';
		}
	}
	public function get_salrry_status_class_user($user_id, $type)
	{
	...
	public function get_salrry_status_class_user1($user_id, $type)
	{
	...
	public function get_salrry_status_advance($user_id, $date_pay)
	{

		$employee_pending_salary = $this->Employees_model->get_pendding_salary_advance($user_id, $date_pay);

		if ($employee_pending_salary) {
			if ($employee_pending_salary->salary > 0) {
				return 'checked disabled';
			} else {
				return '';
			}
		...
		foreach ($payslip->result() as $r) {
			...
			if (count($exit_employee) > 0) {
				...
				if ($exit_date >= $p_date) {
					...
					$field =  '<div class="k-top"><span class="k-icon k-plus" role="presentation"></span><span class="k-checkbox" role="presentation"  style="display:inline-block;"><label><input type="checkbox" ' . $this->get_salrry_status_advance($r->user_id, $p_date) . '  class="role-checkbox user_check_box_adavance"  data-id="' . $r->user_id . '"> </label></span><span class="k-in k-state-focused"></span></div>';
					$field_in =  '<input type="text"  name="b_am" data-id="' . $r->user_id . '" class="form-control advance_class"  style="width: 100px;">';
				}
			} else {
				$emp_name = $r->first_name . ' ' . $r->last_name;
				$full_name = '<a target="_blank" class="text-primary" href="' . site_url() . 'admin/employees/detail/' . $r->user_id . '">' . $emp_name . '</a>';
	...
	public function add_advance_salary()
	{
		...
		foreach ($data as $id) {
			$data_b = array(
				'employee_id' => $id['id'],
				'salary' => $id['adavnce_value'],
				'month' => $month
			);
			$this->db->insert('advance_salary_table', $data_b);
		}
	}
		...
		foreach ($payslip->result() as $r) {
			...
			if (count($exit_employee) > 0) {
				...
				if ($exit_date >= $p_date) {
					...
					$result = $this->Payroll_model->total_hours_worked($r->user_id, $p_date);
					...
					if ($system[0]->is_half_monthly == 1) {
						...
						if ($payment_check->num_rows() > 0) {
							$make_payment = $this->Payroll_model->read_make_payment_payslip($r->user_id, $p_date);
							$mode_of_payment = $make_payment[0]->payment_mode ?? 0;
							$view_url = site_url() . 'admin/payroll/payslip/id/' . $make_payment[0]->payslip_key;
					...
					// $restday_pay1 = $this->Employees_model->restday_pay($r->user_id, $p_date);
					$get_cliam_data = 0;
					// $appeacial_allowance = $this->Employees_model->get_appeacial_allowance($r->user_id, $p_date);
					// $get_cliam_data = $this->Employees_model->get_cliam_data($r->user_id, $p_date);
					...
					$total_earning =  floatval($basic_salary) + floatval($allowance_amount) + floatval($overtime_amount) + floatval($get_cliam_data)  + floatval($restday_pay) + $commissions_amount + $other_payments_amount + $share_options_amount;
					$total_deduction = floatval($loan_de_amount) + $statutory_deductions_amount + $unpaid_leave_amount;
					$total_net_salary = $total_earning - $total_deduction  - $deduction_amount;
					// echo "<pre>";
					// print_r($basic_salary);
					//if($r->salary_advance_paid == ''){
					//$data1 = $add_salary. ' - ' .$loan_de_amount. ' - ' .$net_salary . ' - ' .$salary_ssempee . ' - ' .$statutory_deductions;
					...
					$net_salary = number_format((float)$total_net_salary, 2, '.', '');
					//$basic_salary_cal = $basic_salary * $current_rate; 
					$pay = number_format((float)$basic_salary, 2, '.', '');
					...
					$field =  '<div class="k-top"><span class="k-icon k-plus" role="presentation"></span><span class="k-checkbox" role="presentation"  style="display:inline-block;"><label><input type="checkbox" ' . $this->get_salrry_status_class_user1($r->user_id, 'balance_amount') . '  class="role-checkbox ' . $this->get_salrry_status_class_user($r->user_id, 'balance_amount') . ' "  data-id="' . $r->user_id . '"> </label></span><span class="k-in k-state-focused"></span></div>';
					$column_check_box =  '<span class="k-top"><span class="k-icon k-plus" role="presentation"></span><span class="k-checkbox" role="presentation" style="display:inline-block;"><label><input type="checkbox" class="role-checkbox column_checkbox"  data-id="' . $r->user_id . '"> </label></span><span class="k-in k-state-focused"></span></span>';
				}
			} else {
				// user full name
				$emp_name = $r->first_name . ' ' . $r->last_name;
			...
			$data[] = array(
				...
				$iemp_name,
				$emp_payroll_wage,
				$basic_salary . '<input type="checkbox"   data-id="' . $r->user_id . '" data-type="basic_salary" ' . $this->get_salrry_status($r->user_id, 'gross_salary') . '  class="ml-2 mt-3  ' . $this->get_salrry_status_class($r->user_id, 'gross_salary') . '"/>',
				$restday_pay . '<input type="checkbox"  data-id="' . $r->user_id . '" data-type="restday_pay"  ' . $this->get_salrry_status($r->user_id, 'restday_pay') . ' class="ml-2 mt-3 ' . $this->get_salrry_status_class($r->user_id, 'restday_pay') . '"/>',
				$all_overtime_P . '<input type="checkbox"  data-id="' . $r->user_id . '" data-type="all_overtime_P" ' . $this->get_salrry_status($r->user_id, 'total_overtime') . ' class="ml-2 mt-3 ' . $this->get_salrry_status_class($r->user_id, 'total_overtime') . '"/>',
				$commissions_amount . '<input type="checkbox"  data-id="' . $r->user_id . '"  data-type="commission" ' . $this->get_salrry_status($r->user_id, 'total_commissions') . ' class="ml-2 mt-3 ' . $this->get_salrry_status_class($r->user_id, 'total_commissions') . '"/>',
				$share_options_amount . '<input type="checkbox"  data-id="' . $r->user_id . '" data-type="share_options_amount" ' . $this->get_salrry_status($r->user_id, 'total_share') . '  class="ml-2 mt-3 ' . $this->get_salrry_status_class($r->user_id, 'total_share') . '"/>',
				$other_payments_amount . '<input type="checkbox"  data-id="' . $r->user_id . '" ' . $this->get_salrry_status($r->user_id, 'total_other_payments') . ' data-type="other_payments_amount"  class="ml-2 mt-3 ' . $this->get_salrry_status_class($r->user_id, 'total_other_payments') . '"/>',
				$allowance_amount . '<input type="checkbox"  data-id="' . $r->user_id . '" data-type="allowance_amount" ' . $this->get_salrry_status($r->user_id, 'total_allowances') . '  class="ml-2 mt-3 ' . $this->get_salrry_status_class($r->user_id, 'total_allowances') . '"/>',
				$unpaid_leave_amount . '<input type="checkbox"  data-id="' . $r->user_id . '" ' . $this->get_salrry_status($r->user_id, 'leave_deductions') . '  data-type="unpaid_leave_amount"  class="ml-2 mt-3 ' . $this->get_salrry_status_class($r->user_id, 'leave_deductions') . '"/>',
				$loan_de_amount . '<input type="checkbox"  data-id="' . $r->user_id . '" ' . $this->get_salrry_status($r->user_id, 'total_loan') . ' data-type="loan_de_amount"  class="ml-2 mt-3 ' . $this->get_salrry_status_class($r->user_id, 'total_loan') . '"/>',
				$statutory_deductions_amount . '<input type="checkbox"  data-id="' . $r->user_id . '" ' . $this->get_salrry_status($r->user_id, 'total_statutory_deductions') . ' data-type="statutory_deductions_amount"  class="ml-2 mt-3 ' . $this->get_salrry_status_class($r->user_id, 'total_statutory_deductions') . '"/>',
				$deduction_amount . '<input type="checkbox"  data-id="' . $r->user_id . '" data-type="total_deduction" ' . $this->get_salrry_status($r->user_id, 'deduction_amount') . ' class="ml-2 mt-3 ' . $this->get_salrry_status_class($r->user_id, 'deduction_amount') . '"/>',


				$cpf_employee,
				'<input type="text" id="b_am" name="b_am" class="form-control b_am" value="0" readonly style="width: 100px;">',
				$net_salary,
				$status,
				"",
			);
	...
	public function add_pay_monthly()
	{
		...
		if ($this->input->post('add_type') == 'add_monthly_payment') {
			...
			$this->db->insert('salary_backup_table', $data_bacup);
			$this->db->where('user_id', $this->input->post('u_id'))->update('xin_employees', ['payment_status' => $this->input->post('b_am'), 'payment_type' => 'MO', 'payment_mode' => $this->input->post('payment_mode')]);
			$basic_salary = 0;
			$gross_salary = 0;
			$leave_deductions = 0;
			$total_allowances = 0;
			...
			if ($result) {
				...
				if ($this->input->post('chk_General_amount_oo')) {
	...
	public function payslip()
	{
		...
		// }
		// print_r($result[0]->cpf_employee_amount);die;
		$data['cpf_employer'] = (int)$result[0]->cpf_employer_amount;
		$data['cpf_employee'] = (int)$result[0]->cpf_employee_amount;
		//Contribution funds
