...
class Company extends MY_Controller
{
	...
	public function read()
	{
		...
		$data = array(
			...
			'bank_details'		=> 	$result[0]->bank_details
		);
	...
	public function add_company()
	{
		...
		if ($this->input->post('add_type') == 'company') {
			...
			/* Check if file uploaded..*/ else if ($_FILES['logo']['size'] == 0) {
				$fname = 'no file';
			...
			$bank_details = [];
			foreach ($this->input->post('bank_name') as $key => $name) {
				$bank_details[] = [
					'bank_name'				=>	$name,
					'bank_account_number'	=>	$this->input->post('bank_account_number')[$key],
					'bank_code'				=>	$this->input->post('bank_code')[$key]
				];
			}
	...
	public function add_official_document()
	{
		...
		if ($this->input->post('add_type') == 'official_document') {
			...
			/* Check if file uploaded..*/ else if ($_FILES['scan_file']['size'] == 0) {
				$fname = 'no file';
