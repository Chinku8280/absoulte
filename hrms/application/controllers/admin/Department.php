...
class Department extends MY_Controller {
	 ...
	 public function department_list() {
		...
		$data = array();
          ...
          foreach($department->result() as $r) {
			...
			// get company
			$arr_company_id=explode(",",$r->company_id);
			
			foreach($arr_company_id as $ac){
			$company = $this->Xin_model->read_company_info($ac);
			if(!is_null($company)){
		...
		$comp=implode(",",array_unique($comp_name));
			// get company
	...
	public function add_department() {
		...
		$comp = implode(",",$this->input->post('company_id'));
		$data = array(
		'department_name' => $this->input->post('department_name'),
		'company_id' => $comp,
