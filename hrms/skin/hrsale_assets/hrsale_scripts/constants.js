$(document).ready(function() {		
	...
	var xin_table_claim_type = $('#xin_table_claim_type').dataTable({
		"bDestroy": true,
		"bFilter": false,
		"bLengthChange": false,
		"iDisplayLength": 5,
		"aLengthMenu": [[5, 10, 30, 50, 100, -1], [5, 10, 30, 50, 100, "All"]],
		"ajax": {
            url : site_url+"settings/claim_type_list/",
            type : 'GET'
        },
		"fnDrawCallback": function(settings){
		$('[data-toggle="tooltip"]').tooltip();          
	...
	var xin_table_deduction_type = $('#xin_table_deduction_type').dataTable({
		"bDestroy": true,
		"bFilter": false,
		"bLengthChange": false,
		"iDisplayLength": 5,
		"aLengthMenu": [[5, 10, 30, 50, 100, -1], [5, 10, 30, 50, 100, "All"]],
		"ajax": {
            url : site_url+"settings/deduction_type_list/",
            type : 'GET'
        },
		"fnDrawCallback": function(settings){
		$('[data-toggle="tooltip"]').tooltip();          
	...
	jQuery("#claim_type_info").submit(function(e){
		/*Form Submit*/
		e.preventDefault();
			var obj = jQuery(this), action = obj.attr('name');
			jQuery('.save').prop('disabled', true);
			...
			jQuery.ajax({
				type: "POST",
				url: e.target.action,
				data: obj.serialize()+"&is_ajax=28&data=claim_type_info&type=claim_type_info&form="+action,
				cache: false,
				success: function (JSON) {
					if (JSON.error != '') {
						toastr.error(JSON.error);
						...
						xin_table_claim_type.api().ajax.reload(function(){ 
							toastr.success(JSON.result);
						}, true);
						$('input[name="csrf_hrsale"]').val(JSON.csrf_hash);
						$('.icon-spinner3').hide();
						jQuery('#claim_type_info')[0].reset(); // To reset form fields
						jQuery('.save').prop('disabled', false);
					}
				}
			});
		});
		jQuery("#deduction_type_info").submit(function(e){
			/*Form Submit*/
			e.preventDefault();
				var obj = jQuery(this), action = obj.attr('name');
				jQuery('.save').prop('disabled', true);
				...
				jQuery.ajax({
					type: "POST",
					url: e.target.action,
					data: obj.serialize()+"&is_ajax=28&data=deduction_type_info&type=deduction_type_info&form="+action,
					cache: false,
					success: function (JSON) {
						if (JSON.error != '') {
							toastr.error(JSON.error);
							...
							xin_table_deduction_type.api().ajax.reload(function(){ 
								toastr.success(JSON.result);
							}, true);
							$('input[name="csrf_hrsale"]').val(JSON.csrf_hash);
							$('.icon-spinner3').hide();
							jQuery('#deduction_type_info')[0].reset(); // To reset form fields
							jQuery('.save').prop('disabled', false);
						}
					}
				});
	...
	if(tk_type == 'document_type'){
		...
		else if(tk_type == 'claim_type'){
			var field_add = '&is_ajax=47&data=delete_claim_type&type=delete_record&';
			var tb_name = 'xin_table_'+tk_type;
		}
		else if(tk_type == 'deduction_type'){
			var field_add = '&is_ajax=47&data=delete_deduction_type&type=delete_record&';
			var tb_name = 'xin_table_'+tk_type;
		}
	...
	$('#edit_setting_datail').on('show.bs.modal', function (event) {
		...
		else if(field_type == 'claim_type'){
			var field_add = '&data=ed_claim_type&type=ed_claim_type&';
		}
		else if(field_type == 'deduction_type'){
			var field_add = '&data=ed_deduction_type&type=ed_deduction_type&';
		}
