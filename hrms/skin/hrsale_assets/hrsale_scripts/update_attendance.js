$(document).ready(function() {
    var xin_table = $("#xin_table").dataTable({
        ...
        ajax: {
            url: site_url +
                "timesheet/update_attendance_list/?employee_id=" +
                $("#employee_id").val() +
                "&attendance_date=" +
                $("#attendance_date").val(),
            type: "GET",
    ...
    jQuery("#aj_company").change(function() {
        jQuery.get(
            base_url + "/get_update_employees/" + jQuery(this).val(),
    ...
    // add attendance
    $(".add-modal-data").unbind().on("show.bs.modal", function(event) {
        ...
        $.ajax({
            ...
            type: "GET",
            data: "jd=1&is_ajax=9&mode=modal&data=add_attendance&type=add_attendance&employee_id=" +
                employee_id,
    ...
    $(".edit-modal-data").on("show.bs.modal", function(event) {
        ...
        if (attendance_id != undefined) {
            $.ajax({
                ...
                type: "GET",
                data: "jd=1&is_ajax=1&mode=modal&data=attendance&type=attendance&attendance_id=" +
                    attendance_id,
                success: function(response) {
                    if (response) {
...
$(document).on("click", ".delete", function() {
    ...
    $("#delete_record").attr(
        "action",
        site_url + "timesheet/delete_attendance/" + $(this).data("record-id")
    ) + "/";
});