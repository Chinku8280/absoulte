...
<div class="modal-header">
    <h5 class="modal-title">Quotation Received Payment</h5>
    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
</div>
<div class="modal-body">
    <form class="row text-left" id="quotation_received_payment_form" method="POST" enctype="multipart/form-data">
        @csrf
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="col-md-6">
                            ...
                            <span id="totalAmount">{{$quotation->grand_total}}</span></label>
...
<script>
    ...
    $(document).ready(function () {
        ...
        // $('#quotation_received_payment_form').on('submit', function(e){
        ...
        $('#receive_payment_send_email_modal').find('.select2').select2({
    ...
    {
        var customerId = $("#quotation_received_payment_form").find('#customer_id_lead').val();
        var templateId = $("#receive_payment_send_email_modal").find('#emailTemplateOption').val();

        var temp_invoice_no = "{{$quotation->temp_invoice_no}}";
        // console.log(invoice_no);

        var received_payment_amount = $("#quotation_received_payment_form").find("#received_payment_amount").val() || 0;
        ...
        if(templateId)
        {
            $.ajax({
                ...
                data: {
                    ...
                    'service_address_id': $("#quotation_received_payment_form").find('input[name="service_address"]').val(),
                    'billing_address_id': $("#quotation_received_payment_form").find('input[name="billing_address"]').val(),
                    'quotation_id': $("#quotation_received_payment_form").find("#quotationId").val(),
                    'received_payment_amount' : $("#quotation_received_payment_form").find("#received_payment_amount").val(),
                    'temp_invoice_no': temp_invoice_no,
                },
                success: function(response) {
                    // console.log(response);
                    $("#receive_payment_send_email_modal").find('#send-step-2').html(response);

                    // var email_cc = document.getElementById('email_cc');
                    var email_cc = $("#receive_payment_send_email_modal").find("#email_cc")[0];
                    new Tagify(email_cc);
                    ...
                    ClassicEditor.create(document.querySelector('#receive_payment_send_email_modal #email_body'))
                                .then(editor => {
                    ...
                    // subject
                    var email_subject = $("#receive_payment_send_email_modal").find("#email_subject").val();
                    email_subject = email_subject.replace("##INVOICE_NO##", temp_invoice_no);               
                    $("#receive_payment_send_email_modal").find("#email_subject").val(email_subject);
    ...
    function emailSend(event) 
    {
        ...
        if (window.editor) {
            ...
            $("#receive_payment_send_email_modal").find('#email_body').val(window.editor.getData());
        }

        var email_template_id = $("#receive_payment_send_email_modal").find('#emailTemplateOption').val();
        var email_to = $("#receive_payment_send_email_modal").find('#email_to').val();
        var email_cc = $("#receive_payment_send_email_modal").find('#email_cc').val();
        var email_bcc = $("#receive_payment_send_email_modal").find('#email_bcc').val();
        var email_subject = $("#receive_payment_send_email_modal").find('#email_subject').val();
        var email_body = $("#receive_payment_send_email_modal").find('#email_body').val();

        $("#quotation_received_payment_form").find('#form_email_template_id').val(email_template_id);
        $("#quotation_received_payment_form").find('#form_email_to').val(email_to);
        $("#quotation_received_payment_form").find('#form_email_cc').val(email_cc);
        $("#quotation_received_payment_form").find('#form_email_bcc').val(email_bcc);
        $("#quotation_received_payment_form").find('#form_email_subject').val(email_subject);
        $("#quotation_received_payment_form").find('#form_email_body').val(email_body);

        var data = new FormData($("#quotation_received_payment_form")[0]);

        $.ajax({
            url: "{{ route('quotation.received-payment.send-email') }}",
            method: 'POST',
            data: data,
            processData: false,
            contentType: false,
            beforeSend: function() {
                $("#receive_payment_send_email_modal").find("#email_confirm_btn").attr('disabled', true);
            },
            success: function(response) {
                console.log(response);
            ...
            complete: function() {
                $("#receive_payment_send_email_modal").find("#email_confirm_btn").attr('disabled', false);
            },
